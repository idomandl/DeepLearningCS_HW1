...
def run_sgd_per_hyperparams(loss_cls, lr, batch_size):
    acc_tot = 0
    accuracy_trains, accuracy_tests = [], []
    for data_file in DATA_FILES:
        # print(f'Processing {data_file}')
        _, accuracy_train, accuracy_test = run_sgd(data_file, loss_cls, metric_cls=SoftmaxAccuracy, lr=lr, batch_size=batch_size)
...
if __name__ == "__main__":
    ...
    LOSS_CLS = SoftmaxLoss

    params_combs = [(LOSS_CLS, lr, batch_size) for lr in LEARNING_RATES for batch_size in BATCH_SIZES]
    ...
    fig.suptitle(f'Accuracy, {LOSS_CLS.name}')
