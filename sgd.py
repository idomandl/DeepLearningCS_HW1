...
from softmax_accuracy import SoftmaxAccuracy

class SGD:
    def __init__(self, loss_fn, metric_fn, lr=0.01, stop_condition=0.0001, batch_size=100, loss_sample_size=50):
    ...
    def run(self, D_train, Theta, D_test):
        ...
        while abs(grad_tot - prev_grad_tot) > self.stop_condition:
            ...
            for i in range(0, X_train.shape[0], self.batch_size):
                ...
                self.metric_fn.set_Theta(Theta)
                self.metric_fn.set_Y(Y_train_sample)
                self.loss_fn.set_Theta(Theta)
                self.loss_fn.set_Y(Y_train_sample)
                metric_tot += self.metric_fn(X_train_sample)
                # calculate gradient
                self.loss_fn.set_Theta(Theta)
            ...
            self.metric_fn.set_Theta(Theta)
            self.metric_fn.set_Y(Y_test_sample)
            metric_test.append(self.metric_fn(X_test_sample))
            if len(metric_train) % 200 == 0:
                print(f'{self.metric_fn.name} train: {metric_train[-1]} , {self.metric_fn.name} test: {metric_test[-1]}')
